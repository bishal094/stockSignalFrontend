...
@Component({
  selector: "app-add-crypto-data",
  templateUrl: "./add-crypto-data.component.html",
  styleUrls: ["./add-crypto-data.component.css"],
})
export class AddCryptoDataComponent implements OnInit {
  ...
  AddCrypto() {
    ...
    if (this.cryptoDataDetail["price"] === undefined) {
      ...
      if (this.AddCryptoForm.valid) {
        this.cryptoService
          .addCryptoData(this.AddCryptoForm.value, this.file)
          .subscribe((res) => {
            console.log(res);
            if (res.meta.success) {
